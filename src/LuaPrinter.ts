...
const unescapeStringRegExp = new RegExp(`\\\\${escapeStringRegExp.source}`, "g");
const unescapeStingMap = invertObject(escapeStringMap);
export const unescapeLuaString = (value: string) => {
    assert(value.startsWith('"') && value.endsWith('"'), 'Only strings generated by "escapeString" can be unescaped');
    return value.slice(1, -1).replace(unescapeStringRegExp, char => unescapeStingMap[char]);
};
...
function isSimpleExpression(expression: lua.Expression): boolean {
    switch (expression.kind) {
        case lua.SyntaxKind.CallExpression:
            const calledExpression = (expression as lua.CallExpression).expression;
            // __TS__Resolve macro is guaranteed to be pure
            if (lua.isIdentifier(calledExpression) && calledExpression.text === "__TS__Resolve") {
                return true;
            }
