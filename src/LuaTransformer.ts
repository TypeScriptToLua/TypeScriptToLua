...
interface Scope {
    type: ScopeType;
    id: number;
    locals: tstl.Identifier[];
    functions: tstl.Statement[];
}
...
export class LuaTransformer {
    ...
    public transformScopeBlock(block: ts.Block): tstl.DoStatement {
        ...
        this.popScope(statements);
        return tstl.createDoStatement(statements, undefined, block);
    }
    ...
    public createClassCreationMethods(
        ...
        if (extendsType) {
            ...
            const classVar = this.createLocalOrExportedOrGlobalDeclaration(className, rhs, undefined, statement);
    ...
    public transformEnumDeclaration(enumDeclaration: ts.EnumDeclaration): StatementVisitResult {
        ...
        if (!membersOnly) {
            ...
            result.push(...this.createLocalOrExportedOrGlobalDeclaration(name, table, undefined, enumDeclaration));
        }
    ...
    public transformForOfInitializer(initializer: ts.ForInitializer, expression: tstl.Expression): tstl.Statement {
        if (ts.isVariableDeclarationList(initializer)) {
            ...
            this.popScope(variableDeclarations);
    ...
    private createSelfIdentifier(tsOriginal?: ts.Node): tstl.Identifier {
        return tstl.createIdentifier("self", undefined, tsOriginal);
    }
    ...
    ): tstl.Statement[]
    {
        ...
        if (this.shouldExportIdentifier(lhs)) {
            ...
            if (this.isModule || this.currentNamespace || insideFunction || isLetOrConst) {
                ...
                this.addScopeLocals(lhs, scopeType);
            }
        }

        if (isFunctionDeclaration) {
            // hoist function declarations
            this.addScopeFunction(tstl.createAssignmentStatement(left, rhs, parent, tsOriginal));
            return undefined;
        }

        if (rhs) {
            return [tstl.createAssignmentStatement(left, rhs, parent, tsOriginal)];
        } else {
            return [];
        }
    }
    ...
    protected pushScope(scopeType: ScopeType): void {
        this.scopeStack.push({type: scopeType, id: this.genVarCounter, locals: [], functions: []});
        this.genVarCounter++;
    }
    ...
    protected addScopeLocals(locals: tstl.Identifier | tstl.Identifier[], scopeType?: ScopeType): void {
        let scope: Scope;
        if (!scopeType) {
