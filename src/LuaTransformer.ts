...
export class LuaTransformer {
    ...
    public transformAssignmentStatement(expression: ts.BinaryExpression): tstl.Statement {
        ...
        if (ts.isArrayLiteralExpression(expression.left)) {
            ...
            return tstl.createAssignmentStatement(left as tstl.IdentifierOrTableIndexExpression[], right, undefined, expression);
        } else {
            // Simple assignment
    ...
    public transformAssignmentExpression(expression: ts.BinaryExpression): tstl.CallExpression | tstl.MethodCallExpression {
        // Validate assignment
        const rightType = this.checker.getTypeAtLocation(expression.right);
        const leftType = this.checker.getTypeAtLocation(expression.left);
        this.validateFunctionAssignment(expression.right, rightType, leftType);
    ...
    public transformCompoundAssignmentExpression(
        ...
        if (hasEffects) {
            ...
            if (isPostfix) {
                ...
                const operatorExpression = this.transformBinaryOperation(expression, accessExpression, right, replacementOperator);
            ...
            const operatorExpression = this.transformBinaryOperation(expression, tmpIdentifier, right, replacementOperator);
