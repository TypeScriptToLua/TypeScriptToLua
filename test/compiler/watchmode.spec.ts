...
export class CompilerWatchModeTest {
    @TestCase(["-w", path.join(__dirname, "./testfiles/watch.ts")], path.join(__dirname, "./testfiles/watch.ts"))
    @TestCase(
        ["-w", "-p", path.join(__dirname, "./projects/watchmode/")],
    ...
    public async testSingle(args: string[], fileToChange: string): Promise<void> {
        ...
        await this.waitForFileExists(fileToChangeOut, 9000).catch(err => console.error(err));
        ...
        await this.waitForFileExists(fileToChangeOut, 5000).catch(err => console.error(err));
