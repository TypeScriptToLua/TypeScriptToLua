...
export const checker = (checker: ts.TypeChecker, { value }: { value: any }): ts.TransformerFactory<ts.SourceFile> => (
    context
) => (file) =>
    visitAndReplace(context, file, (node) => {
...
export const compilerOptions = (options: tstl.CompilerOptions): ts.TransformerFactory<ts.SourceFile> => (context) => (
    file
) => {
